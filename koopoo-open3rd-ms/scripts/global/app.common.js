...
(function(me){
	 ...
	 * 容器切换显示
	 * 
	 * @param container 需显示的容器ID或jquery对象
	 * 					'container' 或 '#container' 或 $('#container')
	 * @param options 属性
	 * @returns
	 */
	me.toggleView = function(container, options){
		if(!container) return ;
		var _options = {
			winFlag: 1,// 需隐藏的容器标识(win-flag)值 默认为1
			isTriggerGrid: false// 是否触发jqGrid大小重新调整
		};
		var ops = options ? $.extend(_options, options) : _options;
		
		var reg = '*[win-flag="'+ops.winFlag+'"]';
		$(reg).hide();
		var $_obj = $__me.getJqueryObject(container);
		$_obj.show();
		$('.datetimepicker').hide();//莫名其妙的日期控件的选项框会自动显示出来
		
		if(ops.isTriggerGrid){
			//触发大小调整
		...
		$.teninedialog({
			...
			clickButton: function(sender, modal, index){
				...
				if(index == 1){
					if(ops.okCallback){
						ops.okCallback();
					}else if(ops.callback instanceof Function) {
						ops.callback();
					}
				}else{
	...
	me.show = function(options){
		...
		switch (ops.type) {
			case 1:
				type_ops['class_name'] = 'gritter-success';
				type_ops['image'] = 'gritter-img-success.png'; break;
			case 2:
				type_ops['class_name'] = 'gritter-info';
				type_ops['image'] = 'gritter-img-info.png'; break;
			case 3:
				type_ops['class_name'] = 'gritter-warning';
				type_ops['image'] = 'gritter-img-warning.png'; break;
			case 4:
				type_ops['class_name'] = 'gritter-error';
				type_ops['image'] = 'gritter-img-error.png'; break;
			default://默认为2
				type_ops['class_name'] = 'gritter-info';
				type_ops['image'] = 'gritter-img-info.png'; break;
		}
		...
		$.gritter.add({
			title: ops.title,
			text: ops.content,
			image: config.getFullPath('scripts/gritter/images/'+type_ops.image),
			sticky: false,
			time: ops.time,
			class_name: type_ops.class_name//gritter-error  gritter-error gritter-light
		});
	};
	...
	me.blockUI = function(str){
		if(!loadingCount) {
			...
			wait.fadeIn(300).next().fadeOut(300);
	...
	me.unblockUI = function(mustHide){
		...
		if(--loadingCount < 1) {
			var wait = $__me.getJqueryObject(config.wait);
			// wait.hide(300);
			wait.fadeOut(300).next().fadeOut(300);
			loadingCount = 0;
		}
	};
	 ...
	 * 
	 * @param date 日期对象
	 * @param days 加减天数 正数表示加 负数表示减
	 */
	me.moveDays = function(date, days){
		if(!date) return null;
	 ...
	 * @returns
	 */
	me.moveHours = function(date, hours){
		if(!date) return null;
		var y = date.getFullYear();
		var m = date.getMonth();
		var d = date.getDate();
		var h = date.getHours();
		return new Date(y, m, d, (h + hours));
	};
	 ...
	 * @returns
	 */
	me.moveMinutes = function(date, minutes){
		var y = date.getFullYear();
		var m = date.getMonth();
		var d = date.getDate();
		var h = date.getHours();
		var mm = date.getMinutes();
		return new Date(y, m, d, h, (mm + minutes));
	};
	 ...
	 * 
	 * @param date 日期对象
	 * @param months 加减月数 正数表示加 负数表示减
	 */
	me.moveMonthsForLastDay = function(date, months){
		if(!date) return null;
		var y = date.getFullYear();
		var m = date.getMonth();
	 ...
	 * 
	 * @param date 日期对象
	 * @param years 加减年数 正数表示加 负数表示减
	 */
	me.moveYearsForLastDay = function(date, years){
		if(!date) return null;
		var y = date.getFullYear();
		return new Date(y+years, 13, 0);
	};
	 ...
	 * 替换所有匹配字符串
	 * 
	 * @param targetStr 被替换目标字符串
	 * @param matchStr 匹配字符串
	 * @param replaceStr 即将要替换的新字符串
	 */
	me.strReplaceAll = function(targetStr, matchStr, replaceStr){
		if(targetStr && matchStr && replaceStr){
			return eval("targetStr.replace(/"+matchStr+"/, replaceStr)");
		}else{
			return targetStr;
		}
	};
			...
			switch (error.error) {
				...
				default:
					if(/^Could\s?not\s?read\s?JSON/.test(errorText)) {
						errorText = '数据解析错误！';
					} else if(error.error_description.length < 18) {
 ...
 * */
(function () {
	var $wrapper = $('#formGroupContent');
	var $items = $wrapper.children('.col-xs-6');
	var isMulti = false;
	var showLength = 0;

	if($wrapper.length) {
		showLength = $items.filter(function () {
			return $(this).css('display') == 'block';
		}).length;
		if($items.length > 4 && $items.length > showLength) {
			isMulti = true;
		}
	}

	if(isMulti) {
		var btnDefaultHtml = '显示全部 <i class="glyphicon glyphicon-chevron-down"></i></span>',
			btnStrongHtml = '隐藏部分 <i class="glyphicon glyphicon-chevron-up"></i>';
		$('<span class="btn btn-link form-group-control-bar">'+ btnDefaultHtml +'</span>')
			.appendTo($wrapper.siblings('.search-btn-pack'))
			.on('click', function () {
				if(!$wrapper.hasClass('form-group-show-all')) {
					$(this).addClass('btn-mc-strong').html(btnStrongHtml);
					$wrapper.addClass('form-group-show-all');
				} else {
					$(this).removeClass('btn-mc-strong').html(btnDefaultHtml);
					$wrapper.removeClass('form-group-show-all');
				}
			});
	}

	window.resizeMainSize = function(gridObject) {
		// return false;
		if(!gridObject) gridObject = window.myGridObject;
		if(gridObject) { //jQGrid实例对象
			var $pageMainWrapper = $('#pageMainWrapper');
			gridObject.setGridWidth($pageMainWrapper.width());
		}
	};
