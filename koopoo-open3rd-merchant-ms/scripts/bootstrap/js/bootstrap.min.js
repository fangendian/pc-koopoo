 ...
 */
(function($){$.fn.teninedialog=function(options){var defaults={title:'标题',content:'',url:'',showCloseButton:true,otherButtons:[],otherButtonStyles:[],bootstrapModalOption:{},dialogShow:function(){},dialogShown:function(){},dialogHide:function(){},dialogHidden:function(){},clickButton:function(sender,modal,index){}};var options=$.extend(defaults,options);var modalID='';var loadimg="";if(options.content==''){options.content='正在获取数据...'}function getModalID(){var d=new Date();var vYear=d.getFullYear();var vMon=d.getMonth()+1;var vDay=d.getDate();var h=d.getHours();var m=d.getMinutes();var se=d.getSeconds();var sse=d.getMilliseconds();return't_'+vYear+vMon+vDay+h+m+se+sse}$.fn.extend({closeDialog:function(modal){var modalObj=modal;modalObj.modal('hide')}});return this.each(function(){var obj=$(this);modalID=getModalID();$(this).attr("data-target",modalID);var tmpHtml='<div class="modal fade" style="top:200px;" id="{ID}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel_{ID}" aria-hidden="true"><div class="modal-dialog confirm-modal"><div class="modal-content">'+'<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title" id="myModalLabel_{ID}">{title}</h4></div><div class="modal-body" id="body_{ID}" style="overflow-y: auto;max-height: 600px;">{body}</div><div class="modal-footer"><img id="ajax-loader" style="display:none" height="32" width="32" src="'+loadimg+'" />{button}</div></div></div></div>';var buttonHtml='<button class="btn btn-primary" data-dismiss="modal" aria-hidden="true">关闭</button>';if(!options.showCloseButton&&options.otherButtons.length>0){buttonHtml=''};var btnClass='cls-'+modalID;for(var i=0;i<options.otherButtons.length;i++){buttonHtml+='<button buttonIndex="'+i+'" class="'+btnClass+' btn '+options.otherButtonStyles[i]+'" style="margin-left:10px;">'+options.otherButtons[i]+'</button>'}tmpHtml=tmpHtml.replace(/{ID}/g,modalID).replace(/{title}/g,options.title).replace(/{body}/g,options.content).replace(/{button}/g,buttonHtml);$(obj).append(tmpHtml);var modalObj=$('#'+modalID);$('.'+btnClass).click(function(){var index=$(this).attr('buttonIndex');options.clickButton($(this),modalObj,index)});modalObj.on('show.bs.modal',function(){if(options.url!=''){$('#ajax-loader').show();$.ajax({type:"get",url:options.url,data:"dialogModalId="+modalID+"&ajaxloader=ajax-loader",success:function(data){$('#ajax-loader').hide();$('#body_'+modalID).html(data)},error:function(e){$('#ajax-loader').hide();$('#body_'+modalID).html("加载失败!")}})}else{$('#body_'+modalID).html(options.content)}options.dialogShow()});modalObj.on('shown.bs.modal',function(){options.dialogShown()});modalObj.on('hide.bs.modal',function(){options.dialogHide()});modalObj.on('hidden.bs.modal',function(){options.dialogHidden();obj.removeAttr("data-target");modalObj.remove()});modalObj.modal(options.bootstrapModalOption)})};$.extend({teninedialog:function(options){$("body").teninedialog(options)}})})(jQuery);
