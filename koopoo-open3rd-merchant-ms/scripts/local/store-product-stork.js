...
shopBase.getShopInfo(function (shopInfoData) {
	...
	var pageLoader = new PagesLoader({
		url: 'merchant/shops/'+ shopInfoData.shop_id +'/bps/selectByStock',
	...
	$listTools.children('.list-tools-left').on('click', '#btnListRemove', function () {
			getIdList('确定要删除吗？', function (idList, $trs) {
					ajax.deleteAjaxData({
							url: 'merchant/shops/'+ shopInfoData.shop_id +'/bps/deleteByStock?items='+ idList.join(','),
							success: function () {
									pageLoader.resetThenRequest();
									$trs.remove();
									setToolsButton();
			...
			getIdList('确定要执行上架吗？', function (idList, $trs) {
					ajax.putAjaxData({
							url: 'merchant/shops/'+ shopInfoData.shop_id +'/bps/updateByStock/grounding?items='+ idList.join(','),
							success: function () {
			...
			getIdList('确定要执行下架吗？', function (idList, $trs) {
					ajax.putAjaxData({
							url: 'merchant/shops/'+ shopInfoData.shop_id +'/bps/downdateByStock/undercarriage?items='+ idList.join(','),
							success: function () {
									$trs.removeClass('active');
									pageLoader.resetThenRequest();
									setToolsButton();
...
//判断是否开启拼团，动态显示隐藏拼团配置项
$('#keep-pintuan input').on('switchChange.bootstrapSwitch', function (event,state) {  
	 if(state){
			 $(".showPintuan").css({'display':'block'})
	 }else{
			 $(".showPintuan").css({'display':'none'})
	 }
}); 
...
editObject.$formContainer.find('button.btn-cancel').on('click', function () {
    ...
    * */
	 ...
	 $pin_specificationsValuesTable = $('#pin_specificationsValuesTable');
//选择规格模式
editObject.$formContainer.find(':radio[name=is_unified_spec]').on('change', function () {
	 var value = $(this).val();
	 if(value == 1) { //多规格
			 getSpecificationsAllData();
			 $(".dan_spec").css({"display":"none"})
			 $(".pin_spec").css({"display":"block"})
			 ...
			 $(".dan_spec").css({"display":"block"})
			 $(".pin_spec").css({"display":"none"})
	 }
});
...
function setSpecValuesData() {
	 ...
	 var $pin_html = $(template('pin_specificationsStockTemplate', {
			 list: allListData, titles: titles
	 }));
	 setRowSpanOfTable($pin_html, 0);
	 if(titles.length > 2) setRowSpanOfTable($pin_html, 1);

	 $specificationsValuesTable.html( $html );

	 $pin_specificationsValuesTable.html( $pin_html );
}
...
$('#btnFormSubmit').on('click', function () {
	 ...
	 //验证拼团是否开启
	 var state = $('#keep-pintuan input').bootstrapSwitch('state');
	 if(state){
			 //开启，验证拼团数据，封装
			 var pintuanData = shopBase.getAndVerificationData('#isBulkpurchase', true);
			 if(!pintuanData){
					 $(editObject.idStrHeaderBar).children('[aria-controls=isBulkpurchase]').trigger('click');
					 return;
			 };
			 if(pintuanData.expirationDate>24){
				app.show({
						content: '拼团有效期必须小于24小时', type: 3
				});
				return;
		}
			 // $.extend(true, data, pintuanData);
	 }
	 // console.log('待提交的数据', JSON.stringify(data));
	 var $btn = $(this).attr('disabled', 'disabled');
	 /** 
		* data中存储了pintuan_data 字段，里边包含多规格拼团信息
		* 
		* 
	 */
	 var pintuan_data = data.pintuan_data;
	 if(editObject.editMode === 'new') { //新增
			 ajax.postAjaxData({
					 ...
					 success: function (res) {
							 //执行拼团设置
							 if($('#keep-pintuan input').bootstrapSwitch('state')){
									 pintuanData = $.extend(pintuanData, data.pintuan_data)
									 pintuanData['productId'] = res.product_id;
									 pintuanData.price = parseFloat(pintuanData.price);
									 pintuanData['isBulkpurchase'] == true ? pintuanData['isBulkpurchase'] = 1:pintuanData['isBulkpurchase'] = 0;
									 expiration(pintuanData)
							 }else{
									 pageLoader.resetThenRequest();
									 resetFormContent();
							 }
			 ...
			 ajax.putAjaxData({
					 ...
					 success: function (res) {
							 if($('#keep-pintuan input').bootstrapSwitch('state')){
									 pintuanData = $.extend(pintuanData, data.pintuan_data)
									 pintuanData['productId'] = editObject.productId;
									 pintuanData.price = parseFloat(pintuanData.price);
									 pintuanData['isBulkpurchase'] == true ? pintuanData['isBulkpurchase'] = 1:pintuanData['isBulkpurchase'] = 0;
									 expiration(pintuanData)
							 }else{
									 pintuanNo = editObject.bpinfo;
									 pintuanNo['productId'] = editObject.productId;
									 pintuanNo['isBulkpurchase'] = 0;
									 expiration(pintuanNo)
							 }
					 },
					 complete: complete
			 })
...
function expiration(data){
	 if(editObject.infoId!=undefined){
			 urls = 'merchant/shops/'+ shopInfoData.shop_id +'/bpinfo/'+editObject.infoId
			 ajax.putAjaxData({
					 url: 'merchant/shops/'+ shopInfoData.shop_id +'/bpinfo/'+editObject.infoId,
					 data: data,
					 success: function (res) {
							 if(res.success){
									 pageLoader.resetThenRequest();
			 ...
			 if(!data.isBulkpurchase){
					 pageLoader.resetThenRequest();
						resetFormContent();
					 return;
			 }
			 ajax.postAjaxData({
					 url: 'merchant/shops/'+ shopInfoData.shop_id +'/bpinfo',
					 data: data,
					 success: function (res) {
							 if(res.success){
									 editObject.pt_info = res.data;
									 pageLoader.resetThenRequest();
									 resetFormContent();
							 }else{
									 pageLoader.resetThenRequest();
...
function verificationData(callback) {
	 ...
	 if (is_unified_spec == 1) { //多规格
			 ...
			 if ($selectedSpecLi.length) {
					 ...
					 // 记录拼团多规格数据
					 var pin_priceAndStock = [];
					 var pin_specificationIds = []; //parent-id
					 var pin_specificationValueIds = []; //规格值 value-id _ parent-id


					 $selectedSpecLi.each(function () {
							 specificationIds.push($(this).data('id'));
							 pin_specificationIds.push($(this).data('id'));
					 });

					 $specificationsValuesUl.children('li').each(function () {
							 $(this).find(':checked').each(function () {
									 specificationValueIds.push($(this).data('id') + '_' + $(this).data('s-id'));
									 pin_specificationValueIds.push($(this).data('id') + '_' + $(this).data('s-id'));
							 });
					 });
					 ...
					 $pin_specificationsValuesTable.children('tbody').children('tr').each(function () {
							 var __data = shopBase.getAndVerificationData($(this), true);
							 //获取拼团是否开启  ==> 开启则验证拼团多规格信息
							 if (!__data&&$('#keep-pintuan input').bootstrapSwitch('state')) {
									 pin_error = true;
									 return false;
							 }
							 pin_priceAndStock.push(__data);
					 });
					 
					 if(!error&&!pin_error && priceAndStock.length&&pin_priceAndStock.length) {
							 ...
							 // 收集拼团相关参数
							 pintuan_data.is_unified_spec = false;
							 pintuan_data.priceAndStock = JSON.stringify(pin_priceAndStock);
							 pintuan_data.specificationIds = pin_specificationIds;
							 pintuan_data.specificationValueIds = pin_specificationValueIds;
							 data.pintuan_data = pintuan_data;
					 } else {
							 if(error||!priceAndStock.length){
									 ...
									 return;
							 }else if(pin_error||!pin_priceAndStock.length){
									 app.show({
											 content: '请配置拼团价格!', type: 3
									 });
									 $(editObject.idStrHeaderBar).children('[aria-controls=isBulkpurchase]').trigger('click');
									 $('body').animate({scrollTop: editObject.$formContainer.find('.edit-spec-multiple-box').offset().top - 20}, 200);
							 }else{
	 ...
	 data.product_category_id = editObject.categoriesData[0].product_category_id; //默认
	 ...
	 ajax.getAjaxData({
			 ...
			 success: function (data) {
					 ...
					 * */
					 ...
					 // 查询拼团信息
					 ajax.getAjaxData({
							 url: 'merchant/shops/' + shopInfoData.shop_id + '/bpinfo/' + id,
							 success: function (res) {
									 if(res.data == null){
											 editObject.infoId = undefined;
											 editObject.bpinfo = {};
											 // 未设置开团信息，获取商品列表的信息
											 //多规格数据
											 if(data.product.is_unified_spec) {
													 // editObject.$formContainer.find('input[name=is_unified_spec]').eq(0).trigger('click');
													 // shopBase.setDataBinds(data.data, editObject.$formContainer.find('.edit-spec-single-box'));
													 // editObject.isFirstEdit = false;
											 } else{
													 getSpecificationsAllData(function () {
															 // editObject.$formContainer.find('input[name=is_unified_spec]').eq(1).trigger('click');
															 setOldSpecValue(data, function (allListData, titles) {
																	 // $specificationsValuesUl.html( template('pin_specificationsValuesTemplate', { result: selectedListData }) );
																	 //设置组装表格内容
																	 var $html = $(template('pin_specificationsStockTemplate', {
																			 list: allListData, titles: titles
																	 }));
																	 setRowSpanOfTable($html, 0);
																	 if(titles.length > 2) setRowSpanOfTable($html, 1);
			 
																	 $pin_specificationsValuesTable.html( $html );
																	 editObject.isFirstEdit = false;
															 });
													 });
											 }
									 ...
									 editObject.infoId = res.data.infoId;
									 editObject.bpinfo = res.data;
									 editObject.bpinfo.is_unified_spec = data.product.is_unified_spec;
									 editObject.$formContainer.find('.from-is-checkboxs :checkbox').bootstrapSwitch('state', res.data.isBulkpurchase);
									 shopBase.setDataBinds(res.data, editObject.$formContainer.find('#isBulkpurchase'));
									 if(!res.data.isBulkpurchase){
											 $(".showPintuan").hide();
									 }
									 //多规格数据
									 if(data.product.is_unified_spec==true) {
											 // editObject.$formContainer.find('input[name=is_unified_spec]').eq(0).trigger('click');
											 // shopBase.setDataBinds(data.data, editObject.$formContainer.find('.edit-spec-single-box'));
											 // editObject.isFirstEdit = false;
									 } else if(data.product.is_unified_spec==false) {
											 getSpecificationsAllData(function () {
													 // editObject.$formContainer.find('input[name=is_unified_spec]').eq(1).trigger('click');
													 setOldSpecValue(res.data, function (allListData, titles) {
															 // $specificationsValuesUl.html( template('pin_specificationsValuesTemplate', { result: selectedListData }) );
															 //设置组装表格内容
															 var $html = $(template('pin_specificationsStockTemplate', {
																	 list: allListData, titles: titles
	 ...
	 allListData.forEach(function (item) {
			 newData.stocks.forEach(function (stockItem) {
					 if(stockItem.specification_value == item.specification_value) {
							 ...
							 item.bp_price = stockItem.bp_price;
					 }
			 });
	 });
...
function resetFormContent() {
	 ...
	 $pin_specificationsValuesTable.empty();
					 ...
					 ajax.getAjaxData({
							 ...
							 success: function (res) {
									 if(res.data==null){
											 $("#see_pintuan").hide();
											 return
									 }else if(!res.data.isBulkpurchase){
											 $("#see_pintuan").hide();
											 return;
									 }
									 $("#see_pintuan").show();
									 res.data.isBulkpurchase = true ? '是' : '否';
									 shopBase.setDataBinds(data.res, viewFormContainer);
									 //多规格数据显示
									 if(data.product.is_unified_spec==true) {
											 res.data.is_unified_spec_text = '统一规格';
											 $viewFormContainer.find('.pin_view-spec-single-box').show().siblings('.pin_view-spec-multiple-box').hide().find('table').empty();
									 } else if(data.product.is_unified_spec==false) {
											 res.data.is_unified_spec_text = '多规格';
											 //获取所有规格值
											 getSpecificationsAllData(function () {
													 setOldSpecValue(res.data, function (allListData, titles) {
															 //设置组装表格内容
															 var $html = $(template('pin_viewSpecificationsStockTemplate', {
																	 list: allListData, titles: titles
															 }));
															 setRowSpanOfTable($html, 0);
															 if(titles.length > 2) setRowSpanOfTable($html, 1);
	 
															 $viewFormContainer.find('.pin_view-spec-multiple-box').show().find('table').html( $html );
													 }, true);
											 });
									 }
									 shopBase.setDataBinds(res.data, $viewFormContainer);
							 }
					 });
			 }
	 });
