...
shopBase.getShopInfo(function (shopInfoData) {
	...
	var pageLoader = new PagesLoader({
		url: 'merchant/shops/'+ shopInfoData.shop_id +'/users/list',
		pageBarIdString: '#pageNavWrapper',
		autoLoad: true,
		success: function (data) {
            data.result.forEach(function (item) {
                item.sex = dict.getSexForHtml_B(item.sex);
                item.area = item.country + "-" + item.province + "-" + item.city;
	...
	function showView(id) {
		if(!id) return;
		ajax.getAjaxData({
			url: 'merchant/shops/' + shopInfoData.shop_id + '/users/' + id,
			success: function (data) {
				data.sex = dict.getSexForHtml_B(data.sex);
				data.area = data.country + "-" + data.province + "-" + data.city;
