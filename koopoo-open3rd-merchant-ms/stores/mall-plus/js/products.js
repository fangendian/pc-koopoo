...
shopBase.getShopInfo(function (shopInfoData) {
    ...
    $listTools.on('click', '#btnListOnline', function () {
    ...
    $listTools.on('click', '#btnListOffline', function () {
    ...
    * */
    $('#btnFormSubmit').on('click', function () {
        ...
        var data = __verificationData;
        if(!data) {
            data = verificationData(); //收集基本信息
            if(!data) {
                $(idStrHeaderBar).children('[aria-controls=baseInfoBox]').trigger('click');
                return;
            }
    ...
    * */
    function resetFormContent() {
        ...
        $formContainer.find(idStrHeaderBar).children('[aria-controls=categoriesBox]').addClass('active').siblings().removeClass('active').addClass('disabled');
