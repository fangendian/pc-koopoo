...
shopBase.getShopInfo(function (shopInfoData) {
    ...
    var pageLoader = new PagesLoader({
        url: 'merchant/shops/'+ shopInfoData.shop_id +'/teachers/list',
    ...
    $listTools.on('click', '#btnListRemove', function () {
        getIdList('确定要禁用吗？', function (idList, $trs) {
            ajax.deleteAjaxData({
                url: 'merchant/shops/'+ shopInfoData.shop_id +'/teachers?items='+ idList.join(','),
                success: function () {
                    pageLoader.resetThenRequest();
                    $trs.remove();
                    setToolsButton();
    ...
    var editObject = {
        ...
        editMode: null, //'new':新增, 'edit':修改
        postId: null,
        categoryId: null,
        isChange: false, //内容是否更改，用于取消提醒
        isFirstEdit: false //第一次编辑
    };
    ...
    $('#btnFormSubmit').on('click', function () {
        if(editObject.editMode === 'edit' && !editObject.postId) return;
        ...
        if(editObject.editMode === 'new') { //新增
            ajax.postAjaxData({
                url: 'merchant/shops/'+ shopInfoData.shop_id +'/teachers',
                data: data,
                success: function () {
                    pageLoader.resetThenRequest();
                    resetFormContent();
            ...
            ajax.putAjaxData({
                url: 'merchant/shops/'+ shopInfoData.shop_id +'/teachers/'+ editObject.postId,
                data: data,
                success: function () {
                    pageLoader.resetThenRequest();
                    resetFormContent();
    ...
    function showEdit(id){
        ...
        ajax.getAjaxData({
            url: 'merchant/shops/' + shopInfoData.shop_id + '/teachers/' + id,
            success: function (data) {
                // console.log('get product:', data);
                editObject.editMode = 'edit';
                editObject.postId = id;
                ...
                shopBase.setDataBinds(data, editObject.$formContainer.find('.group-base-wrapper'));
    ...
    function resetFormContent() {
        ...
        editObject.$formContainer.find('input:checkbox').each(function () {
