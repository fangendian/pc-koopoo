...
$(function () {
	...
	nav.init({
		...
		callback: function (shopInfoData) {
			...
			if (this.TEMPLATE_ID[0] === templateId || this.TEMPLATE_ID[1] === templateId) {
				if (this.TEMPLATE_ID[1] === templateId) {
					shopBase.removeArrayItemByKV(this.data, 1);
				}
				this.initPage();
			} else {
				this.backToList();
